   ...
   execute: async (auxdibot: Auxdibot, interaction: AuxdibotCommandInteraction<GuildAuxdibotCommandData>) =>
      {
         ...
         if (server.suggestions_updates_channel) {
            const channel: GuildBasedChannel | undefined = await interaction.data.guild.channels.fetch(server.suggestions_updates_channel).catch(() => undefined);
            ...
            const channel: GuildBasedChannel | undefined = await interaction.data.guild.channels.fetch(server.suggestions_channel).catch(() => undefined);
            if (channel && channel.isTextBased()) {
               const embed = JSON.parse(
                  await parsePlaceholders(
                     auxdibot,
               ...
               const message: Message<boolean> | undefined = await channel.messages.fetch(suggestion.messageID).catch(() => undefined);
               if (message) {

...
export default <AuxdibotSubcommand>{
   ...
   info: {
      ...
      description: 'Unrequire a role required to use a command.',
      usageExample: '/commands role unrequire (command) (role)',
   },
      ...
      await updateCommandPermissions(
         ...
         .then(async (data) => {
            ...
            embed.description = `The role <@&${
               role.id
            }> is no longer required to use the command \`/${commandStr.replace(/^\//g, '')}\`.`;

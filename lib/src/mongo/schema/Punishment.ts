...
export function toEmbedField(punishment: IPunishment): EmbedField {
    return <EmbedField>{
        ...
        value: `🕰️ Date: <t:${Math.round(punishment.date_unix / 1000)}>\n${ punishment.expired ? "📅 Expired" : `📅 Expires: ${!punishment.expires_date_unix ? "Never" : `<t:${Math.round(punishment.expires_date_unix / 1000)}>`}`}\n💬 Reason: ${punishment.reason}\n⛓️ User: <@${punishment.user_id}>\n🧍 Moderator: ${punishment.moderator_id ? `<@${punishment.moderator_id}>` : "None"}`
    }
}

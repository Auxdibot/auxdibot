...
const getGuildData = async (guild_info: GuildInfo) => {
   const guildData = await Server.findOne({ discord_id: guild_info.id }, { _id: 0 })
      .exec()
      .catch(() => undefined);
   if (guildData) {
      const data = await guildData.fetchData(),
         settings = await guildData.fetchSettings();
...
guildRouter.get('/:guildId', validateUser, async (req: express.Request, res: express.Response) => {
   ...
   const guilds = req.session.passport.user.discord_guilds;
   const guild = guilds.filter((guild) => guild.id == req.params['guildId'])[0];
...
guildRouter.get('/', validateUser, async (req: express.Request, res: express.Response) => {
   ...
   return res
      .status(200)
      .json(
         await Promise.all(req.session.passport.user.discord_guilds.map(async (guild) => await getGuildData(guild))),
      );
});

    ...
    subcommands: [{
        ...
        async execute(interaction: AuxdibotCommandInteraction<GuildAuxdibotCommandData>) {
            ...
            try {
                ...
                if (interaction.channel && interaction.channel.isTextBased()) await interaction.channel.send({ content: `Here's a preview of the new join embed!\n${settings.join_dm_text || ""}`, embeds: [JSON.parse(await parsePlaceholders(JSON.stringify(settings.join_embed), interaction.data.guild, interaction.data.member)) as APIEmbed] });
            } catch (x) {
                let embed = Embeds.ERROR_EMBED.toJSON();
                embed.description = "Couldn't make that embed!";
                return interaction.channel && interaction.channel.isTextBased() ? await interaction.channel.send({ embeds: [embed] }) : undefined;
            }
            return;
        }
    },
        {
            ...
            async execute(interaction: AuxdibotCommandInteraction<GuildAuxdibotCommandData>) {
                ...
                try {
                    ...
                    embed.title = "Success!";
                    embed.description = `Set the join embed.`;
                    if (interaction.channel && interaction.channel.isTextBased()) await interaction.channel.send({ content: "Here's a preview of the new join embed!", ...(Object.entries(settings.join_embed || {}).length != 0 ? { embeds: [JSON.parse(await parsePlaceholders(JSON.stringify(settings.join_embed), interaction.data.guild, interaction.data.member)) as APIEmbed] } : {})});
        ...
        },
        {
            ...
            async execute(interaction: AuxdibotCommandInteraction<GuildAuxdibotCommandData>) {
                ...
                let settings = await interaction.data.guildData.fetchSettings();
                try {
                    return await interaction.reply({ content: `**EMBED PREVIEW**\r\n${settings.join_text || ""}`, ...(Object.entries(settings.join_embed || {}).length != 0 ? { embeds: [JSON.parse(await parsePlaceholders(JSON.stringify(settings.join_embed), interaction.data.guild, interaction.data.member)) as APIEmbed] } : {})});
                } catch (x) {
                    console.log(x);

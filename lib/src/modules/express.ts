...
type UserType = {
   ...
   discord_guilds: DiscordStrategy.GuildInfo[];
};
...
export class AuxdibotAPI {
   async init() {
               ...
               User.findOneAndUpdate({ discord_id: profile.id }, {}, { upsert: true, new: true })
                  .then((data) => {
                     ...
                     return cb(undefined, <UserType>{
                        ...
                        discord_guilds: profile.guilds
                           ? profile.guilds.filter((guild) => guild.owner || (guild.permissions & 0x08) == 0x08)
                           : [],
                     });
                  })
      ...
      const whitelist = ['https://bot.auxdible.me', 'http://localhost:5173'];
      app.use(
         cors({

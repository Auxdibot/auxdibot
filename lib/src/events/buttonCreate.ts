...
module.exports = {
    ...
    async execute(interaction: MessageComponentInteraction) {
        ...
        if (client.buttons) {
            let button = client.buttons.get(interaction.customId.split("-")[0]);
            if (button) {
                if (button.permission) {
                    if (!server.testPermission(button.permission, interaction.member as GuildMember, button.allowedDefault || false)) {
